...
import org.jgroups.util.DebugLogger;
...
public class ClientSessionFactoryImpl implements ClientSessionFactoryInternal, ConnectionLifeCycleListener
{
   ...
   private static final DebugLogger logger = DebugLogger.getLogger("test.log");
   ...
   {
      logger.log("in getConnectionWithRetry(), attempts: " + reconnectAttempts);
      if (exitLoop)
         return;
      ...
      while (!exitLoop)
         {
            ...
            logger.log("calling getConnection()...");
            getConnection();
            logger.log("done, connection now: " + connection);
            ...
            if (connection == null)
            {
               ...
               if (reconnectAttempts != 0)
               {
                  ...
                  logger.log("increasing count: " + count);
                  if (reconnectAttempts != -1 && count == reconnectAttempts)
                  {
                     ...
                     logger.log("either count reaches max or -1, abandoning...");
                     return;
                  }
                  ...
                  try
                  {
                     logger.log("waiting before retry: " + interval);
                  if (waitLatch.await(interval, TimeUnit.MILLISECONDS))
                     return;
                  logger.log("waiting done, go on");
                  }
               ...
               {
                  logger.log("don't have reconnect attempts configured, just abandon");
            ...
            else
            {
               ...
               logger.log("connection ok, return");
               return;
            }
         }
   }
   ...
   public CoreRemotingConnection getConnection()
   {
      ...
      if (connection == null)
      {
         ...
         try
         {
            ...
            logger.log("Trying to connect with connector = " + connectorFactory +
                                                  ", parameters = " +
            ...
            if (connector != null)
            {
               ...
               logger.log("connector created a connection: " + tc);
            ...
            if (connector == null)
            {
               logger.log("see if we have backup: " + backupConfig);
               if (backupConfig != null)
               {
                  if (ClientSessionFactoryImpl.isDebug)
                  {
                  ...
                  logger.log("yeah got connector from backup: " + connector);

                  if (connector != null)
                  {
                     connector.start();
                     logger.log("backup connector started.");

                     tc = connector.createConnection();

                     logger.log("bk created tc: " + tc);
         ...
         {
            logger.log("got exception", false, e);
         ...
         if (tc == null)
         {
            ...
            logger.log("return conn as tc is null: " + connection);
            return connection;
         }
      ...
      logger.log("New connection successfully created: " + connection);
      return connection;
      }
   }
